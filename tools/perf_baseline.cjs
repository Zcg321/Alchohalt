...
function writeBaseline(measurement, mode = 'update') {
  /* [R13-FIXUP-2-COPILOT] Mode-aware notes so a bootstrapped
   * baseline self-describes correctly instead of always claiming
   * "via --update". Codex review on PR #47 flagged the inconsistency. */
  const noteByMode = {
    update:
      'Auto-generated by tools/perf_baseline.cjs --update. Regenerate on every merge to main.',
    bootstrap:
      'Auto-generated by tools/perf_baseline.cjs --bootstrap-or-update on a missing-baseline run. Regenerate on every merge to main.',
  };
  const blob = {
    ...measurement,
    notes: noteByMode[mode] ?? noteByMode.update,
  };
...
if (!baseline) {
  ...
  if (arg === '--bootstrap-or-update') {
    ...
    writeBaseline(current, 'bootstrap');
    process.exit(0);
  }
  console.error(
