...
function handleSalvage(raw: unknown) {
  try {
    ...
    // localStorage / Blob unavailable — fall back to copying to the clipboard.
    try {
...
function ActionButton({ onClick, testId, title, body, variant = 'neutral' }: ActionButtonProps) {
  const danger = variant === 'danger';
  const cls = danger
    ? 'w-full rounded-2xl border border-rose-200 bg-white px-5 py-3.5 text-start text-sm font-medium text-rose-700 hover:bg-rose-50 dark:border-rose-900/50 dark:bg-rose-950/30 dark:text-rose-300 dark:hover:bg-rose-950/50 transition-colors min-h-[48px]'
    : 'w-full rounded-2xl border border-neutral-200/70 bg-white px-5 py-3.5 text-start text-sm font-medium text-neutral-800 hover:bg-neutral-50 dark:border-neutral-700/60 dark:bg-neutral-800/60 dark:text-neutral-100 dark:hover:bg-neutral-800 transition-colors min-h-[48px]';
  const bodyCls = danger
    ? 'text-xs font-normal text-rose-600/80 dark:text-rose-400/70 mt-0.5'
    : 'text-xs font-normal text-neutral-500 dark:text-neutral-400 mt-0.5';
  return (
...
function ConfirmFreshDialog({ onConfirm, onCancel }: { onConfirm: () => void; onCancel: () => void }) {
  return (
    <div
    ...
    >
      ...
      <div className="text-xs text-rose-700/80 dark:text-rose-300/70 mt-1">
        If you haven&rsquo;t saved a salvage copy yet, do that first.
      </div>
...
function RecoveryActions({ raw }: { raw: unknown }) {
  const [confirmingFresh, setConfirmingFresh] = useState(false);
  return (
    <div className="mt-5 grid gap-2.5">
      <ActionButton
        onClick={handleTryAgain}
        testId="recovery-try-again"
        title="Try again"
        body="Reload the app and re-read the saved file."
      />
      <ActionButton
        onClick={() => handleSalvage(raw)}
        testId="recovery-salvage"
        title="Save what we can"
        body="Download the raw file as JSON. Open it in any text editor; pull out anything intact."
      />
      {!confirmingFresh ? (
        <ActionButton
          ...
          title="Start fresh"
          body="Clear the corrupt file and start over. This deletes everything that couldn’t be loaded."
          variant="danger"
        />
      ) : (
        <ConfirmFreshDialog
...
export default function DataRecoveryScreen() {
  ...
  useEffect(() => subscribeCorruption(setEvent), []);
