...
export function calculateAchievementProgress(
  ...
  const achievements: Achievement[] = BASE_ACHIEVEMENTS.map(base => {
    ...
    switch (base.id) {
      ...
      case 'goal-crusher':
        // This would need a more sophisticated goal tracking system
        current = 0;
        break;
      ...
      case 'craving-conqueror':
        current = drinks.filter(d => d.craving <= 2).length;
        break;
      ...
      case 'zen-master':
        ...
        const avgCraving = last30Days.length > 0 
          ...
          : 0;
        current = avgCraving <= 1.0 && last30Days.length >= 30 ? 30 : 0;
        break;

      case 'social-butterfly':
        current = drinks.filter(d => d.intention === 'social' && d.alt).length;
        break;

      case 'influence-expert':
        current = drinks.filter(d => d.intention === 'social').length;
        break;
      ...
      case 'money-saver':
        current = Math.floor(savedMoney);
        break;

      case 'weekend-warrior':
        // Check for alcohol-free weekends
        const weekends = getConsecutiveAlcoholFreeWeekends(drinks);
        current = weekends;
        break;
