...
function PassphraseSection({ passphrase, setPassphrase, busy, onVerify, onReset }: {
  passphrase: string; setPassphrase: (s: string) => void;
  busy: boolean; onVerify: () => void; onReset: () => void;
}) {
  return (
    <>
      <div className="space-y-1">
        <label htmlFor="backup-verify-pass" className="label">Passphrase</label>
        <input
          id="backup-verify-pass" type="password" value={passphrase}
      ...
      <div className="flex flex-wrap gap-2">
        <button type="button" onClick={onVerify} disabled={busy || !passphrase}
          className="btn btn-primary" data-testid="backup-verifier-verify-btn">
...
function SuccessReport({ report }: { report: VerifyBackupReport & { ok: true } }) {
  return (
    <div role="status" data-testid="backup-verifier-success"
...
function ErrorReport({ report }: { report: VerifyBackupReport & { ok: false } }) {
  return (
    <div role="alert" data-testid="backup-verifier-error"
      className="rounded-lg border border-red-300 bg-red-50 px-4 py-3 text-sm dark:border-red-700 dark:bg-red-950/50">
      <p className="font-medium text-red-900 dark:text-red-100">{report.error}</p>
      {report.hint && <p className="mt-1 text-red-800 dark:text-red-200">{report.hint}</p>}
    </div>
  );
}

function useBackupVerifier() {
  ...
  async function onPickFile(ev: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setFileBody(text); setFileName(f.name);
    } catch {
      setReport({ ok: false, error: "Couldn't read the file.",
  ...
  async function onVerify() {
    ...
    try { setReport(await verifyBackup(fileBody, passphrase)); }
    catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      setReport({ ok: false, error: 'Verification crashed unexpectedly.', hint: message });
    } finally { setBusy(false); }
  }
  function reset() { setFileBody(null); setFileName(''); setPassphrase(''); setReport(null); }
  return { fileBody, fileName, passphrase, setPassphrase, busy, report, onPickFile, onVerify, reset };
}

export default function BackupVerifier() {
  const v = useBackupVerifier();
  return (
    <section className="card" data-testid="backup-verifier">
      <div className="card-header">
        <h3 className="text-base font-semibold tracking-tight">Verify a backup file</h3>
        <p className="text-sm text-neutral-600 dark:text-neutral-400 mt-1">
          Confirms a .alch-backup file unlocks with your passphrase and the contents are intact — without changing anything on this device. Run this after every backup you save.
        </p>
      </div>
      <div className="card-content space-y-4">
        <div>
          <label className={`btn btn-secondary ${v.fileBody ? 'opacity-50' : ''}`}>
            {v.fileBody ? `Picked: ${v.fileName}` : 'Pick a .alch-backup file'}
            <input type="file" className="hidden"
              accept=".alch-backup,application/octet-stream"
              onChange={v.onPickFile}
              data-testid="backup-verifier-file-input"
              aria-label="Backup file to verify" />
          </label>
        </div>
        {v.fileBody && (
          <PassphraseSection passphrase={v.passphrase} setPassphrase={v.setPassphrase}
            busy={v.busy} onVerify={v.onVerify} onReset={v.reset} />
        )}
        {v.report?.ok && <SuccessReport report={v.report} />}
        {v.report && !v.report.ok && <ErrorReport report={v.report} />}
      </div>
    </section>
  );
}
