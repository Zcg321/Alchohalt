...
const preferencesStorage: StateStorage = {
  getItem: async (name) => (await Preferences.get({ key: name })).value ?? null,
  setItem: async (name, value) => { await Preferences.set({ key: name, value }); },
  removeItem: async (name) => { await Preferences.remove({ key: name }); },
};
  ...
  persist(
    (set, get) => {
      ...
      return {
        ...
        editEntry(id: UUID, patch: Partial<Entry>) {
          const db = {
            ...get().db,
        ...
        setLanguage(language: Language) { const db = { ...get().db, settings: { ...get().db.settings, language } }; set({ db }); get()._recompute(); },
        ...
        dismissReminderUntil(ts: number) { const db = { ...get().db, meta: { ...get().db.meta, reminderSuppressedUntil: ts } }; set({ db }); },
        wipeAll(confirmFlag: boolean) { if (!confirmFlag) return; const db = defaults(); set({ db }); get()._recompute(); },
        _recompute() {
